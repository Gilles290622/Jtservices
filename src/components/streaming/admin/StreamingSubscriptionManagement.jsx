...
const StreamingSubscriptionManagement = () => {
    ...
    <div>
      <div className="overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {loading ? (
              <TableRow><TableCell colSpan={6} className="text-center p-8">Chargement des demandes...</TableCell></TableRow>
            ) : (
              subscriptions.map(sub => (
                <TableRow key={sub.id} className="hover:bg-gray-800/50 border-gray-700">
                  <TableCell>
                    <div className="font-medium">{sub.profile?.full_name}</div>
                    <div className="text-sm text-gray-400">{sub.profile?.username}</div>
                  </TableCell>
...
export default StreamingSubscriptionManagement;